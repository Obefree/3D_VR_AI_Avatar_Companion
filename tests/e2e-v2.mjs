...
try {
  ...
  assert.equal(indexSource.includes('__NOVA_PRIMARY_FETCH'), false, 'PRIMARY_FETCH paper-over is still loaded');
  assert.equal(/window\.fetch\s*=/.test(embodimentSource), false, 'embodiment fetch interceptor would run actions beside app.js');
  ...
  await page.waitForSelector('#cinematic-director', { timeout: 15000 });
  ...
  assert.ok(
    hit.tag === 'CANVAS' || hit.pointerEvents === 'none',
    `orbit gesture is blocked by ${hit.tag}#${hit.id}.${hit.className} pointer-events=${hit.pointerEvents}`,
  );
