...
assert.equal(scenarioSource.includes('collapseConsecutive'), true, 'consecutive duplicate actions are not collapsed');
assert.equal(indexSource.includes('tap-interaction.js'), false, 'legacy tap bridge is still loaded');
assert.equal(binocularSource.includes('requestSession'), false, 'binocular VR still starts a second XR session beside xr-controls');
...
try {
  ...
  assert.ok(initial.scenarioActions.includes('wait'), 'scenario wait action missing');
  ...
  assert.ok(analysis.dialogueExamples.some((line) => /Привет/.test(line) && /здесь/.test(line)), 'quoted multi-sentence dialogue not extracted as one line');
  ...
  const chains = await page.evaluate(async (value) => {
    ...
    const merged = window.__novaScenarioCore.mergePlans(main, [
      { name: 'approach_user', args: { distance: 2 } },
      { name: 'step', args: { direction: 'front', distance: 1 } },
      { name: 'speak', args: { text: 'AI duplicate line' } },
    ...
    return {
      ...
      closer: names(closer),
      device: names(device),
      dialogue: main.actions.some((action) => action.name === 'speak' && /Привет\. Я здесь/.test(action.args?.text || '')),
      dualMovers: main.actions.filter((action) => ['approach_user', 'move_near', 'step'].includes(action.name)).map((action) => action.name),
      mergedMovers: merged.actions.filter((action) => ['approach_user', 'move_near', 'step'].includes(action.name)).map((action) => action.name),
      mergedSpeak: merged.actions.filter((action) => action.name === 'speak').map((action) => action.args?.text),
      mergedWave: merged.actions.filter((action) => action.name === 'wave').map((action) => action.args?.side),
    };
  }, script);
  ...
  assert.ok(chains.main.includes('wait'), 'pause beat did not plan wait');
  assert.equal(chains.dialogue, true, 'multi-sentence dialogue not preserved');
  assert.deepEqual(chains.dualMovers, ['approach_user'], 'more than one mover planned for the viewer approach');
  ...
  assert.deepEqual(chains.mergedMovers, ['approach_user'], 'AI merge reintroduced a parallel mover');
  assert.deepEqual(chains.mergedSpeak, ['Привет. Я здесь'], 'AI merge replaced local dialogue');
  assert.deepEqual(chains.mergedWave, ['left', 'right'], 'AI extra non-mover action was dropped');
  ...
  assert.ok(result.results.some((item) => item.action.name === 'speak' && item.result?.ok), 'speak did not execute');
  assert.ok(result.results.some((item) => item.action.name === 'wave' && item.result?.ok), 'wave did not execute');
  assert.ok(result.results.some((item) => item.action.name === 'wait' && item.result?.ok), 'wait did not execute');
