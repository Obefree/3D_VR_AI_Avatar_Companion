...
assert.match(scenarioSource, /collapseConsecutive/, 'consecutive duplicate actions are not collapsed');
...
try {
  ...
  const dispatchers = await page.evaluate(() => ({
    ...
    fetchLooksHijacked: String(window.fetch).includes('extendedActions'),
  }));
  assert.equal(dispatchers.appExecute, 'function', 'app executeAction not exported');
  assert.equal(dispatchers.fetchLooksHijacked, false, 'window.fetch still looks like an action interceptor');
  ...
  const chains = await page.evaluate(async (value) => {
    ...
    const english = await window.__novaScenarioCore.compile('Come closer and wave.', { ai: false });
    const device = await window.__novaScenarioCore.compile('Она подходит к устройству.', { ai: false });
    const pronoun = await window.__novaScenarioCore.compile('Смотрит на устройство. Затем указывает на него.', { ai: false });
    return {
      main: names(main),
      mainKeys: main.actions.map((action) => `${action.name}:${JSON.stringify(action.args || {})}`),
      closer: names(closer),
  ...
  for (let i = 1; i < chains.mainKeys.length; i += 1) {
    assert.notEqual(chains.mainKeys[i - 1], chains.mainKeys[i], `consecutive duplicate action ${chains.mainKeys[i]}`);
  }
  assert.ok(chains.closer.includes('approach_user'), 'imperative подойди ближе did not plan approach_user');
  assert.ok(chains.english.includes('approach_user'), 'come closer did not approach the viewer');
  assert.ok(chains.english.includes('wave'), 'wave missing from come closer chain');
  ...
  assert.ok(chains.pronoun.actions.some((action) => action.name === 'look_at' && action.args?.targetId === 'device'), 'device look_at missing');
  assert.ok(chains.pronoun.actions.some((action) => action.name === 'point_at' && action.args?.targetId === 'device'), 'pronoun did not keep device target');
  ...
  await page.evaluate(() => {
    ...
    window.__novaEmbodimentExecutions = 0;
    window.__novaEmbodiment.execute = async (action) => {
      window.__novaEmbodimentExecutions += 1;
