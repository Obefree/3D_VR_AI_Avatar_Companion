...
try {
  ...
  const chains = await page.evaluate(async (value) => {
    ...
    return {
      ...
      deviceDual: device.actions.some((action) => action.name === 'approach_user')
        && device.actions.some((action) => action.name === 'move_near'),
    };
  }, script);
  ...
  assert.equal(chains.deviceDual, false, 'device approach planned parallel movers');
