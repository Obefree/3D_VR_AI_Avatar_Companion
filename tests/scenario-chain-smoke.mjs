...
assert.equal(indexSource.includes('character-analyzer.js'), false, 'unwired character-analyzer duplicate is loaded');
...
await assert.rejects(access(resolve(root, 'src/character-analyzer.js')), /ENOENT/, 'orphan character-analyzer duplicate remains');
...
try {
  ...
  const dispatchers = await page.evaluate(() => ({
    ...
    fetchIsNative: !String(window.fetch).includes('extendedActions'),
  }));
  ...
  assert.equal(dispatchers.fetchIsNative, true, 'window.fetch still looks like an action interceptor');
  ...
  const waved = await page.evaluate(async () => {
    const before = window.__novaEmbodimentExecutions;
    ...
    return window.__novaEmbodimentExecutions - before;
  });
