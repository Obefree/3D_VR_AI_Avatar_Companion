...
assert.match(director, /async function dispatchShared/, 'shared NovaApp dispatcher missing');
assert.match(director, /walk_to.*move_near|move_near.*walk_to/s, 'named-object walk must reuse move_near');
assert.equal(director.includes('async function walkToTarget'), false, 'duplicate walkToTarget locomotion remains');
...
assert.equal(index.includes('__NOVA_PRIMARY_FETCH'), false, 'PRIMARY_FETCH paper-over must stay removed');

const embodiment = fs.readFileSync(new URL('../src/embodiment.js', import.meta.url), 'utf8');
assert.equal(/window\.fetch\s*=/.test(embodiment), false, 'embodiment fetch interceptor would run actions in parallel with app.js');
assert.equal(fs.existsSync(new URL('./e2e.mjs', import.meta.url)), false, 'stale tests/e2e.mjs duplicate remains');
