...
function mockReply(body) {
  ...
  const toolFailure = (body.toolResults || []).find((item) => item?.result?.ok === false);

  if (body.phase === 'after_tools' && toolFailure?.result?.error === 'reset_required') {
    return {
      ok: true,
...
try {
  ...
  // B1. Natural-language show: should point/highlight but NOT press.
  ...
  assert.equal(state.reset, false, 'show/highlight accidentally pressed the reset button');

  // B2. Pronoun follow-up + visible button depression + persistent reset state.
  ...
  assert.ok(pronounRequest, 'pronoun request never reached AI backend');
  assert.ok(
  ...
  // B4. Filter removal must physically move the filter and complete task state.
  ...
  const lastText = await page.locator('#messages .message.assistant').last().innerText();
  assert.match(lastText, /завершена|готово|да/i);

  // C. Fresh-state precondition: a physical filter tap cannot bypass reset.
  ...
  // Put all targets comfortably on screen before physical hit tests (equivalent to zooming out).
  await page.evaluate(() => {
    const s = window.__novaScene;
    s.camera.position.set(0, 1.7, 7.2);
  ...
  // D1. Actual mobile tap on red button center -> one response and real state transition.
  await page.evaluate(() => window.__novaScene.resetTask());
  let red = await projectTarget('red_button');
  assert.ok(red.x > 0 && red.x < 390 && red.y > 0 && red.y < 844, 'red button not in mobile viewport');
  ...
  // D3. Shell tap does not recolor the whole device.
  ...
  beforeTapMessages = await page.locator('#messages .message').count();
  await page.touchscreen.tap(device.x - 28, device.y - 18);
  await page.waitForFunction(
    (count) => document.querySelectorAll('#messages .message').length >= count + 2,
    beforeTapMessages,
    { timeout: 12000 },
  ...
  // E. Actual touch rotate + two-finger pinch on the mobile canvas.
  ...
  // F. Voice wiring with mocked SpeechRecognition: transcript must use the same /api/chat path.
  ...
  await page.waitForFunction(
    () => window.__novaScene.lookTarget === 'red_button',
    null,
    { timeout: 12000 },
  );
  ...
  console.log('E2E PASS: connect -> Russian semantic actions -> persistent task state -> physical taps -> touch orbit/pinch -> mocked voice path');
} finally {
  await browser?.close();
