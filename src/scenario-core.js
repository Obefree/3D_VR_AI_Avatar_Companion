(() => {
  ...
  function actionsForBeat(beat, runtimeScene, actorProfile, context) {
    ...
    const approaches = /подход|подойд|приближ|поближе|(^|[^\p{L}\p{N}_])ближе([^\p{L}\p{N}_]|$)|ид[её]т к|walks? (?:up|to|closer)|comes? closer|come closer|step closer|approach/iu.test(text);
    ...
    if (approaches) {
      if (targetId && !explicitViewer) pushUnique(actions, { name: 'move_near', args: { targetId } });
  ...
  function aiActionConflicts(existing, incoming) {
    if (incoming.name === 'speak') return true;
    if (incoming.name === 'approach_user' && (hasAction(existing, 'approach_user') || hasAction(existing, 'move_near'))) return true;
    if (incoming.name === 'move_near' && (hasAction(existing, 'move_near') || hasAction(existing, 'approach_user'))) return true;
    if (incoming.name === 'step' && incoming.args?.direction === 'front' && (hasAction(existing, 'approach_user') || hasAction(existing, 'move_near'))) return true;
    return false;
  }
  ...
  function mergePlans(localPlan, aiActions) {
    ...
    for (const aiAction of aiActions) {
      if (aiActionConflicts(actions, aiAction)) continue;
      pushUnique(actions, aiAction);
    }
    return { ...localPlan, source: 'ai+core', actions: collapseConsecutive(actions) };
  }
  ...
  async function aiCompile(script, localPlan) {
    ...
    const message = [
      ...
      'Use approach_user for the viewer. Use move_near for a named scene object. Never plan both for the same approach.',
